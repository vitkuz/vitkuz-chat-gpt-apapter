...
async function testDefaults() {
    ...
    if (res1._receivedInput.model !== CHAT_GPT_MODELS.GPT_4o)
        throw new Error('Model default failed');
    ...
    if (res2._receivedInput.model !== CHAT_GPT_MODELS.GPT_4o_MINI)
        throw new Error('Model override failed');
    if (res2._receivedInput.temperature !== 0.5)
        throw new Error('Default temperature should have been kept');
    ...
    if (res3._receivedInput.model !== CHAT_GPT_MODELS.GPT_4o)
        throw new Error('Default model should have been kept');
    ...
    if (res5._receivedInput.text?.format?.name !== 'test_schema')
